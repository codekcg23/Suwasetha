				...
				<div class="row">
					<div class="col-xs-12">
						<div class="top">
							<div class="row">
								<div class="col-md-6">
									<div id="colorlib-logo"><a href="index.html"><i class="flaticon-stethoscope"></i>Suwasetha Medi<span>care</span></a></div>
								</div>
								<div class="col-md-3">
									<div class="num">
...
if(isset($_GET['Employee_No'])){
	...
	$sql="DELETE FROM Nurse WHERE Employee_No= '$del_id'";
	if(mysqli_query($conn,$sql)){
		echo "DEleted";
	};
}
$sql="SELECT Nurse.Employee_No,Employee.Name,Grade,Salary,Surgery_Code FROM Nurse,Employee WHERE Nurse.Employee_No=Employee.Employee_No";

	

$records= mysqli_query($conn,$sql);	

echo "<table border='0' width='100%' cellspacing='10px' bgcolor='#b3ffb3' style='margin-left:20px;margin-bottom:50px;' >";
echo "<tr><th>Employee_No</th><th>Name</th><th>Grade</th><th>Salary</th><th>Surgery_Code</th></tr>";
while($employee= mysqli_fetch_assoc($records)){
	
	
echo "<tr><td>{$employee['Employee_No']}</td><td>{$employee['Name']}</td><td>{$employee['Grade']}<td>{$employee['Salary']}<td>{$employee['Surgery_Code']}</td><td class='cell100 column6'><p class='line-block'><a href='services.php?Employee_No={$employee['Employee_No']}' class='btn btn-primary btn-outline btn-cta'>DELETE</a></p></td><td class='cell100 column6'><p class='line-block'><a href='updateform.php?Employee_No={$employee['Employee_No']}' class='btn btn-primary btn-outline btn-cta'>UPDATE</a></p></td></tr>";
...
?>
	...
	<p align="right"><a href="doctors-single.html" class="btn btn-primary btn-send-message btn-md" >Back to Profile</a></p>
